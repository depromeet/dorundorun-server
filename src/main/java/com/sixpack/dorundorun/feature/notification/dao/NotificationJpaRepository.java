...
public interface NotificationJpaRepository extends JpaRepository<Notification, Long> {
	...
	// 사용자의 알림 목록 조회 (페이징)
	@Query("SELECT n FROM Notification n WHERE n.recipientUserId = :userId AND n.deletedAt IS NULL ORDER BY n.createdAt DESC")
	Page<Notification> findByRecipientUserIdAndDeletedAtIsNull(
		@Param("userId") Long userId,
		Pageable pageable
	);

	// 읽지 않은 알림 개수 조회
	@Query("SELECT COUNT(n) FROM Notification n WHERE n.recipientUserId = :userId AND n.isRead = false AND n.deletedAt IS NULL")
